...
if len(sys.argv) != 7:
    print("Usage: python script.py input_file output_file first_ax_name second_ax_name third_ax_name fourth_ax_name")
    sys.exit(1)
...
ax1.plot(x, y3, '^-', label=third_ax_name, color='lightgreen', markersize=10)
...
ax2.bar([i + 3 * bar_width for i in x[zoom_start:zoom_end]], y4[zoom_start:zoom_end], label=fourth_ax_name, color='firebrick', width=bar_width)  # Shift x values for Dataset 4
