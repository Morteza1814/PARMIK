...
# Initialize empty lists for x, y2, y3, and y4
x = []
y2 = []
y3 = []
y4 = []
...
if len(sys.argv) != 6:
    print("Usage: python script.py input_file output_file second_ax_name third_ax_name fourth_ax_name")
    sys.exit(1)
...
with open(input_file, 'r') as file:
    for line in file:
        ...
        y4.append(float(data[3]))  # Convert y4 value to float
...
ax1.plot(x, y3, '^-', label=third_ax_name, color='skyblue', markersize=10)
ax1.plot(x, y4, 'v-', label=fourth_ax_name, color='firebrick', markersize=10)
...
ax2.bar([i + bar_width for i in x[zoom_start:zoom_end]], y3[zoom_start:zoom_end], label=third_ax_name, color='skyblue', width=bar_width)  # Shift x values for Dataset 3
ax2.bar([i + 2 * bar_width for i in x[zoom_start:zoom_end]], y4[zoom_start:zoom_end], label=fourth_ax_name, color='firebrick', width=bar_width)  # Shift x values for Dataset 4
