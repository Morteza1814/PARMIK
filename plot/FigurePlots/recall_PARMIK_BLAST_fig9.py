...
def plotFig(x, y2, y3, y4, output_file):
    second_ax_name = "PARMIK"
    third_ax_name = "BLAST"
    fourth_ax_name = "BOWTIE"
    ...
    ax1.plot(x, y2, '-', label=second_ax_name, color='navy', markersize=10)
    ax1.plot(x, y3, '-', label=third_ax_name, color='firebrick', markersize=10)
    ax1.plot(x, y4, '-', label=fourth_ax_name, color='darkgreen', markersize=10)
    ...
    # Plot the zoomed-in portion as a bar chart in the bottom subplot
    ax2.bar(x_zoom, y2[zoom_start:zoom_end], label=second_ax_name, color='navy', width=bar_width)
    ax2.bar([i + bar_width for i in x_zoom], y3[zoom_start:zoom_end], label=third_ax_name, color='firebrick', width=bar_width)
    ax2.bar([i + 2 * bar_width for i in x_zoom], y4[zoom_start:zoom_end], label=fourth_ax_name, color='darkgreen', width=bar_width)
...
def calculate_recall_rate(tpArray, fnArray):
    ...
    for i in range(len(tpArray)):
        ...
        if denominator > 0:
            ...
            recall_rate_array[i] = 0
    return recall_rate_array

def getRecallOutOfTpFn(tpFilePath, fnFilePath):
    aln_len, tp = read_first_and_last_column(tpFilePath)
    _, fn = read_first_and_last_column(fnFilePath)
    tp_cum = cumulative_sum_from_each_element(tp)
    fn_cum = cumulative_sum_from_each_element(fn)
    recall = calculate_recall_rate(tp_cum, fn_cum)
    np.set_printoptions(suppress=True)
...
blastTpFilePath = experimentPath + '/cmp_Baseline_blast_AlnSz_tp.txt'
blastFnFilePath = experimentPath + '/cmp_Baseline_blast_AlnSz_fn.txt'
bowtieTpFilePath = experimentPath + '/cmp_Baseline_bowtie_AlnSz_tp.txt'
bowtieFnFilePath = experimentPath + '/cmp_Baseline_bowtie_AlnSz_fn.txt'
output_file = experimentPath + '/PARMIKvsBLASTvsBOWTIE_recall.pdf'

aln_len, parmikRecall = getRecallOutOfTpFn(parmikTpFilePath, parmikFnFilePath)
_, blastRecall = getRecallOutOfTpFn(blastTpFilePath, blastFnFilePath)
_, bowtieRecall = getRecallOutOfTpFn(bowtieTpFilePath, bowtieFnFilePath)

plotFig(aln_len[10:150], parmikRecall[10:150], blastRecall[10:150], bowtieRecall[10:150], output_file)

for a, p, b, bt in zip(aln_len[10:150], parmikRecall[10:150], blastRecall[10:150], bowtieRecall[10:150]):
    print(f"{a:<10} {p:<15.2f} {b:<15.2f} {bt:<15.2f}")
