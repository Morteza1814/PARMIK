...
for metagenomicDataset in metagenomicDatasets:
    for queryDataset in queryDatasets:
        datasetName = ('Wuhan-Hu-1' if queryDataset == 'NC_045512' else queryDataset) + ' vs. ' + metagenomicDataset
        print('=========================================')
        print(datasetName)
        ...
        for m in M:
            ...
            for i, pi in enumerate(PI):
                ...
                if m == 7:
                    ax.scatter(parmik_recall[i], parmik_time[i], label='PARMIK [M=7]', color='dodgerblue')
                elif m == 4:
                    ax.scatter(parmik_recall[i], parmik_time[i], label='PARMIK [M=4]', color='navy')
                    ...
                    bowtie_recall.append(getRecallOutOfTpFn(bowtieTpFilePath, bowtieFnFilePath, R))
                    bowtie_speed.append(getSpeedOutOfTp(bowtieTpFilePath, bowtieTimePath, R))
                    bowtie_time.append(getTime(bowtieTimePath))
                    ax.scatter(bowtie_recall[i], bowtie_time[i], label='Bowtie', color='darkgreen')
        ...
        print('BLAST:')
        print('blast_recall: ', blast_recall)
        print('blast_time (Minutes): ', blast_time)
        print('blast_speed: ', blast_speed)

        print('BOWTIE:')
        print('bowtie_recall: ', bowtie_recall)
        print('bowtie_time (Minutes): ', bowtie_time)
        print('bowtie_speed: ', bowtie_speed)
