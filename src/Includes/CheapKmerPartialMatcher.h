...
public:
    ...
    IndexContainer<queryIndT, readIndT>& minThCheapSeeds)
    {
        ...
        for(auto itq = queries.begin(); itq != queries.end(); itq++) {
            ...
            map<readIndT, uint32_t> cheapSeedReads;
            start = chrono::high_resolution_clock::now();
            uint32_t cheapSeeds = cheapKmers.collectCheapSeeds(queryRegionKmers, cheapSeedReads);
            queriesNumCheapKmers.insert(cheapSeeds);
            ...
            queriesMinThCheapSeeds.insert(minThCheapSeedsOfQuery.size());
        }
    ...
    void collectReadsWithMinThCheapSeedsNumber(map<readIndT, uint32_t> cheapSeedReads, IndexContainer<queryIndT, readIndT> &minThCheapSeedReads, queryIndT queryInd)
    {
